...
@section('content')
<div class="container">
    <h1>Modifier une Plage Horaire de Films</h1>
    @include('partials.messages')
    <form action="{{ route('filmSchedules.updateFilm', ['schedule' => $schedule->id, 'film' => $filmId ?? 'new']) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <input type="datetime-local" class="form-control" id="start_time" name="start_time" value="{{ $schedule->start_time }}" required>
        </div>

        <div class="form-group">
            <label for="end_time">Heure de fin</label>
            <input type="datetime-local" class="form-control" id="end_time" name="end_time" value="{{ $schedule->end_time }}" required>
        </div>
